...
import KatalogManualWorkflowClient from "@/app/(dashboard)/katalog/KatalogManualWorkflowClient";
...
function ModuleAction({
  ...
  // Katalog tidak membuat paket baru. Sumber paket berasal dari RUP/SIRUP
  // yang sudah tayang dengan metode E_PURCHASING.
  if (moduleKey === "katalog") return null;
  if (pageKey === "paket") {
...
function katalogProcessLabel(status?: string | null) {
  if (!status) return "Siap Diproses";

  const normalized = status.toUpperCase();
  if (normalized.includes("SELESAI")) return "Selesai";
  if (normalized.includes("SERAH")) return "BAST / Serah Terima";
  if (normalized.includes("PELAKSANAAN")) return "Pengiriman / Pelaksanaan";
  if (normalized.includes("KONTRAK")) return "Surat Pesanan / Kontrak";
  if (
    normalized.includes("PEMENANG") ||
    normalized.includes("PEMILIHAN") ||
    normalized.includes("EVALUASI")
  ) {
    return "Pemilihan / Negosiasi";
  }

  return humanize(status);
}

function katalogProcessStyle(status?: string | null) {
  ...
  if (
    ...
    normalized.includes("PELAKSANAAN") ||
    normalized.includes("SERAH")
  ) {
...
async function KatalogWorkflowView({
  searchParams = {},
}: {
  searchParams?: Record<string, string | string[] | undefined>;
}) {
  ...
  // Pada schema file saat ini belum terlihat foreign-key langsung RUP -> Paket.
  // Karena itu status proses yang sudah ada dicocokkan secara aman berdasarkan
  // nama paket + unit. Ketika schema sudah punya rupId, ganti mapping ini ke rupId.
  const packageByRupKey = new Map(
    existingPackages.map((item) => [
      `${normalizeUnit(item.namaPaket)}::${normalizeUnit(item.unitPemohon)}`,
      item,
    ]),
  );

  const getLinkedPackage = (item: (typeof rupRows)[number]) =>
    packageByRupKey.get(
      `${normalizeUnit(item.namaPaket)}::${normalizeUnit(item.unitPengusul)}`,
    );

  const totalPagu = rupRows.reduce(
    (sum, item) => sum + decimalNumber(item.pagu),
    0,
  );

  const linkedRows = rupRows.map((rup) => ({
    rup,
    paket: getLinkedPackage(rup),
  }));

  const readyCount = linkedRows.filter(({ paket }) => !paket).length;
  const doneCount = linkedRows.filter(({ paket }) =>
      ...
      <form className="border-b border-slate-200 bg-white px-4 py-3 sm:px-6 lg:px-8">
        <div className="flex min-w-0 flex-wrap items-center gap-2">
          ...
          <select
            name="sumberDana"
            defaultValue={sumberDana}
            className="h-9 min-w-[210px] rounded-md border border-slate-300 bg-white px-3 text-sm font-semibold text-slate-600"
          >
            <option value="">Semua Sumber Dana</option>
            {sourceFunds.map((item) => (
              <option key={item.kode} value={item.kode}>
          ...
          <select
            name="unitPengusul"
            defaultValue={unitPengusul}
            className="h-9 min-w-[190px] rounded-md border border-slate-300 bg-white px-3 text-sm font-semibold text-slate-600"
          >
            <option value="">Semua Unit</option>
            {units.map((item) => (
              <option key={item.unitPengusul} value={item.unitPengusul}>
      ...
      <div className="space-y-5 px-4 py-5 sm:px-6 lg:px-8">
        <section className="rounded-lg border border-emerald-200 bg-emerald-50 px-5 py-4">
          <div className="flex items-start gap-3">
            <ShoppingCart className="mt-0.5 h-5 w-5 shrink-0 text-[#08783f]" />
            <div>
              <p className="text-sm font-black text-[#08783f]">
                Paket Katalog berasal dari RUP, bukan diinput ulang
              </p>
              <p className="mt-1 text-sm font-semibold leading-6 text-slate-600">
                Halaman ini hanya menampilkan RUP yang sudah tayang dan
                menggunakan metode e-Katalog/e-Purchasing. Data produk dan
                penyedia dicatat manual di aplikasi internal tanpa API LKPP:
                pilih produk, pilih penyedia, negosiasi, surat pesanan,
                pengiriman, BAST, lalu pembayaran.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {[
            ...
            },
            {
              ...
              helper: "Dari RUP/SIRUP",
              tone: "border-l-[#43a047]",
            },
            {
              label: "Siap Diproses",
              value: readyCount.toLocaleString("id-ID"),
              helper: "Belum masuk proses pemilihan",
              tone: "border-l-[#f57c00]",
            },
            {
            ...
            <table className="min-w-[1100px] w-full border-collapse text-left text-sm">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50 text-xs font-black uppercase text-slate-400">
                  ...
                  <th className="px-4 py-3">Status e-Katalog</th>
                  <th className="px-4 py-3 text-right">Aksi</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
...
async function CustomStaticModuleView({
  pageKey,
  searchParams = {},
}: {
  pageKey: string;
  ...
  if (pageKey === "katalog") {
    return <KatalogWorkflowView searchParams={searchParams} />;
  }
...
async function renderProcurementModulePage({
  ...
  const customView = await CustomStaticModuleView({
    pageKey,
    searchParams,
  });
