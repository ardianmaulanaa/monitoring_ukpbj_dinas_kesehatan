...
function rupMutationData(parsedData: z.infer<typeof createRupSchema>) {
  ...
  return {
    ...
    volumeKebutuhan: nullableText(volumeKebutuhan),
...
export async function POST(request: Request) {
  ...
  try {
    const rup = await prisma.rencanaUmumPengadaan.create({
      data: rupMutationData(parsed.data),
    });
...
export async function PUT(request: Request) {
  ...
  if (!canDeletePlanningProposal(user.roles)) {
    return apiError("Hanya superadmin yang boleh mengubah data perencanaan/RUP.", 403);
  }
  ...
  try {
    ...
    return apiSuccess(rup, "Data RUP berhasil diubah.");
  } catch (error) {
    if (
      error instanceof Prisma.PrismaClientKnownRequestError &&
      error.code === "P2002"
