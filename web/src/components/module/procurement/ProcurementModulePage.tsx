...
// Konfigurasi judul/header tiap halaman module. Dipakai oleh renderProcurementModulePage().
const pageConfigs = {
  dashboard: {
    title: "Dashboard",
    subtitle:
...
// Alias menyambungkan route detail/tambah/edit ke konfigurasi module induknya.
...
function resolveConfig(pageKey: string) {
  // Cari konfigurasi halaman dari pageKey, alias, atau fallback ke dashboard.
...
export async function renderProcurementModulePage({
  pageKey,
  tahap,
  searchParams = {},
}: RenderProcurementModulePageProps) {
  // pageKey dikirim dari route app/*/page.tsx untuk menentukan halaman module mana yang dirender.
  const config = resolveConfig(pageKey);
  const isProcurementStage = config.title === "Tahapan Pengadaan";

  // Halaman user punya view khusus karena datanya bukan tabel module biasa.
  if (
    pageKey === "admin-users" ||
  ...
  // Halaman role punya view khusus untuk matrix hak akses.
  ...
  // Cabang khusus tahapan pengadaan: perencanaan, pemilihan, atau dashboard tahap umum.
  if (isProcurementStage) {
    ...
    // Data dashboard tahapan diambil dari lib/dashboard-data.ts.
    const dashboard = await getDashboardData();
  ...
  // Beberapa module punya tampilan custom sendiri, bukan ModuleListView generik.
  ...
  return (
    <>
      {/* Fallback umum: header atas + list/table module generik. */}
      <AppHeader
        title={config.title}
